...
show_number = 10
# AD correct
fig_ac, axes_ac = plt.subplots(2, show_number)
# # NC correct
...
def show_slices(img_file, axes, type, index):
    ...
    axes[type][index].imshow(slice_2.T, cmap="gray", origin="lower")
    axes[type][index].axis('off')
    ...
    if y_true[i] == 0:
        if y_true[i] == y_pred[i]:
            if AD_correct_number >= show_number:
                ...
                show_slices(x_set[i], axes_ac, 0, AD_correct_number)
                AD_correct_number += 1
        else:
        ...
        if y_true[i] == y_pred[i]:
            if NC_correct_number >= show_number:
                ...
                show_slices(x_set[i], axes_ac, 1, NC_correct_number)
                NC_correct_number += 1
        else:
